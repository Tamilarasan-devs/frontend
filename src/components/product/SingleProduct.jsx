...
import img6 from '../../assets/images/produt/6.png'
// ─── Static Data ──────────────────────────────────────────────────────────────
const PACKS = [
...
const Pill = ({ children, className = "" }) => (
  <span
    className={`inline-flex items-center gap-1 text-[12px] font-semibold px-3 py-1 rounded-full border ${className}`}
  >
    {children}
  </span>
);
  ...
  return (
    <div className="border-b border-stone-200">
      ...
      >
        {title}
        <span
          className={`text-black/50 text-sm transition-transform duration-200 flex-shrink-0 ml-4 ${
            open ? "rotate-180" : ""
          }`}
        >
          ▾
      ...
      >
        <div className="pb-5 text-[14px] font-medium text-black leading-relaxed">
          {children}
        </div>
      </div>
    </div>
      ...
      <nav className="bg-white border-b border-stone-200 px-4 sm:px-6 lg:px-10 py-3 flex items-center flex-wrap gap-1.5 text-[13px] font-semibold text-black">
        {["Home", "Wellness", "Milk Masala"].map((c) => (
          <span key={c} className="flex items-center gap-1.5">
            <a href="#" className="text-black/50 hover:text-red-900 transition-colors font-medium">
              {c}
            </a>
            <span className="text-black/30">›</span>
          </span>
        ))}
        <span className="font-bold text-red-900">Brain Tonic</span>
      </nav>
    ...
    >
      ...
      />
            ...
            <div className="bg-gray-100 rounded-lg p-4">
              <p className="text-md font-bold text-black uppercase tracking-widest mb-2.5">
                Available Offers
              </p>
              <div className="flex gap-2.5 overflow-x-auto pb-1">
                {OFFERS.map(({ icon, bg, title, body }) => (
                  ...
                  >
                    <div className={`w-10 h-10 ${bg} rounded-full flex items-center justify-center text-sm mb-2`}>
                      {icon}
                    </div>
          ...
          <div className="flex flex-col">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              <Pill className="bg-green-50 text-green-900 border-green-300">✓ Brain</Pill>
              <Pill className="bg-blue-50 text-blue-900 border-blue-300">✓ Memory</Pill>
              <Pill className="bg-amber-50 text-amber-900 border-amber-300">✓ Focus</Pill>
            </div>
            ...
            <p className="text-[15px] font-semibold text-black mb-4">
              Enhances memory, focus, and brain performance.
            </p>
            ...
            {/* Pack Selector */}
            <div className="mb-6">
              <p className="text-[12px] font-bold text-black uppercase tracking-widest mb-3">
                Choose Pack Size
              </p>
                ...
                {PACKS.map((p, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-[16px] font-bold text-black">₹{p.price}</p>
                      <p className="text-[11px] font-semibold text-black/40 line-through">₹{p.orig}</p>
...
<button
  ...
  style={{
    // background: "linear-gradient(135deg, #820c0c, #b91c1c)",
    boxShadow: "0 10px 24px rgba(130,12,12,.35)"
  }}
>
  <svg
    className="w-5 h-5"
  ...
  <span>Add to Cart</span>
            ...
            <div className="grid grid-cols-2 gap-2.5 mb-5">
              {HIGHLIGHTS.map(({ icon, label }) => (
                <div
                  key={label}
                  className="flex items-center gap-2.5 px-4 py-3 rounded-xl border border-stone-200 bg-white text-[13px] font-bold text-black hover:border-stone-300 transition-colors"
                >
              ...
              {[
                ...
                <div
                  ...
                  className={`flex flex-col items-center gap-1 py-4 px-2 text-center ${
                    i < 2 ? "border-r-2 border-stone-200" : ""
                  }`}
                >
        ...
        {activeTab === "description" && (
          <div className="max-w-2xl">
            <p className="text-[18px] font-medium text-black leading-relaxed mb-6">
              Enriched with a carefully curated selection of traditional Ayurvedic herbs, Quista Active Milk
              Masala has been crafted to naturally enhance vitality — delivering the wisdom of ancient wellness
              in every sip.
            </p>
              ...
              <ul className="space-y-3 text-lg ">
                {BENEFITS.map((b) => (
                  <li key={b} className="flex gap-3 items-start">
                    <span className="w-5 h-5 rounded-full bg-red-900 flex items-center justify-center flex-shrink-0 mt-0.5 text-[9px] text-white font-bold">
                      ✓
                    </span>
                    <span>{b}</span>
                  </li>
          ...
          <div className="max-w-2xl">
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {ingredientList.map((ing) => (
                <span
                  key={ing}
                  className="text-[12px] font-bold px-3.5 py-1.5 rounded-full bg-amber-50 text-amber-900 border border-amber-300"
                >
                  {ing}
                </span>
              ))}
            ...
            <div className="rounded-2xl p-5 bg-red-50 border border-red-200">
              ...
              <p className="text-[14px] font-medium text-black leading-relaxed">
                May contain traces of nuts. Consult a healthcare professional if pregnant, nursing, or on
                medication.
              </p>
            </div>
          </div>
